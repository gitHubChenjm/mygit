...
<input type="hidden" id="user" value="${sessionScope.user.id}">
	...
	<script type="text/javascript">
			...
			$.ajax({
						...
						success : function(data) {
							...
							$.each(result.list,function(idx, obj) {
												...
												var preOption = '';
												var nextOption ='';
												if(statusId == '73'){//若为已分配的，就执行归档
													preOption = ' layui-btn-xs layui-btn-normal" onclick="doOnFileService(';
													nextOption =')">归档</button></td>';
													total++;
												}else if(statusId =='74'){//若已归档，就执行查看归档
													preOption = ' layui-btn-xs" onclick="seeOnFileService(';
													nextOption =')">查看</button></td>';
													total++;
												}else{
													return true;//相当于continuce 
												}
												var str = "<tr>"
														+ "<td>"
														+ obj.name
														...
														+ "</td>"
														+ '<td><button type="button" class="layui-btn' + preOption  
														+ "'"
														+ obj.serviceId
														+ "'"
														+ nextOption
		...
		//去到归档页面
		function doOnFileService(id){
			
			Page.open('服务归档','service/onFileService.action?id='+id);
		}
		
		//去到归档查看页面，查看详细信息
		function seeOnFileService(id){
			Page.open('查看归档详情','service/seeOnFileService.action?id='+id);
		}

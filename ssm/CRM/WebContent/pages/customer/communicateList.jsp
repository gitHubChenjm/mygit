...
<form id="form" class="layui-form">
		...
		<input id="customerId" type="hidden" value="${communicates.customer.customerId}"/>
		<div class="layui-form-item">
			<label class="layui-form-label">客户编号:</label>
			<div class="layui-input-inline">
				<input type="text" name="customer.customerId" value="${communicates.customer.customerId}" readonly="readonly"
					class="layui-input">
			</div>
			<label class="layui-form-label">客户名称:</label>
			<div class="layui-input-inline">
				<input type="text" value="${communicates.customer.name}" readonly="readonly" class="layui-input">
			</div>
		</div>
		<div class="layui-form-item">
			<label class="layui-form-label">开始时间:</label>
			<div class="layui-input-inline">
				<input type="text" name="startCreateDate" placeholder="请输入交往开始时间"
			...
			<div class="layui-input-inline">
				<input type="text" name="endCreateDate" placeholder="请输入交往截止时间"
					class="layui-input Wdate" id="endCreateDate"
					onfocus="WdatePicker({dateFmt:'yyyy-MM-dd',readOnly:true,minDate:'#F{$dp.$D(\'startDate\')}'})">
			</div>
			<label class="layui-form-label">地点:</label>
			<div class="layui-input-inline">
				<input type="text" name="address" placeholder="请输入交往地点" id="address"
					class="layui-input">
			</div>
		...
		<div class="layui-form-item more-search">
			...
			<div class="layui-input-inline">
				<input type="text" name="generel" placeholder="请输入交往概要" id="generel"
					class="layui-input">
			</div>
			<label class="layui-form-label">详细信息:</label>
			<div class="layui-input-inline">
				<input type="text" name="detail" placeholder="请输入详细信息" id="detail" class="layui-input">
			</div>
			<label class="layui-form-label">备注:</label>
			<div class="layui-input-inline">
				<input type="text" name="remark" placeholder="请输入备注" id="remark" class="layui-input">
			</div>
		</div>
		
	</form>
	<div class="layui-form-item">
			<span class="ft-green ft-28">${communicates.customer.name} <span class="ft-warm ft-28">的</span><span class="ft-red ft-28"> 交往记录</span></span>
	</div>
	<div class="layui-btn-group">
		<button class="layui-btn"
			onclick="Page.open('新建交往记录','customers/addCommunicate.action?customerId=${communicates.customer.customerId}')">
		...
		<table class="layui-table" id="my-table">
			<thead>
				<tr class="layui-bg-gray">
					...
					<th width="10%"><span class="ft-bold">交往时间</span></th>

					<th width="15%"><span class="ft-bold">地点</span></th>

					<th width="10%"><span class="ft-bold">概要</span></th>

					<th width="15%"><span class="ft-bold">详细信息</span></th>

					<th width="15%"><span class="ft-bold">备注</span></th>
					
					<th width="5%"><span class="ft-bold">记录文件</span></th>
	...
	<script type="text/javascript">
			function destory(id) {
				confirm("确定删除交往记录？", function() {
					$.ajax({
						url : "customers/delCommunicate.action",
						dataType : "text",
						type : "POST",
						async : false,
						data : {"communicateId" : id},
			...
			$('#delAll').click(function(){
				...
				if(list.length == 0){//一条数据没选择，可不易删，不然可能有空指针错误
					...
					window.location.href="customers/tolistCommunicate.action?customer.customerId=" + idb;
				}else{
					var l = list.length;
					confirm("确定删除所选的"+l+"个交往记录？", function() {
						$.ajax({
							url : "customers/delMoreCommunicate.action",
							dataType : "text",
							type : "POST",
							async : false,
							...
							success : function(data){
								...
								if (r.msg == 1) {
									...
									window.location.href="customers/tolistCommunicate.action?customer.customerId=" + idd;
								} else {
				                    parent.layer.msg("删除失败！", {icon: 5});
				                }
							}
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			$.ajax({
				url : 'customers/listCommunicateDate.action',
				type : 'post',
				async : false,
				data : {},
				success : function(data) {
					...
					$.each(result.communicates, function(idx, obj) {
		...
		function findList() {
			...
			$.ajax({
				url : 'customers/listCommunicate.action',
				type : 'post',
				async : false,
					...
					$.each(
							...
							function(idx, obj) {
								...
								if(f == null || f == ''){
									...
									//如果有文件的话就提供下载
									var af = '<a href="customers/download.action?fileName=' + obj.file + '"><span class="ft-red">文件下载</span></a>';
									f = af;
								}
								...
								+ f
								+ "</td>"
								+ '<td><button id="update" class="layui-btn" onclick="Page.open(' + "'修改交流记录'" + "," + "'customers/editCommunicate.action?communicateId="
								+ obj.communicateId
								+ "')" + '">'
		...
		$(function () {
		    $("[type='reset1']").click(function () {
		        ...
		        $(this).parents(".layui-form").find("input #address").val("");
		        $(this).parents(".layui-form").find("input #general").val("");
		        $(this).parents(".layui-form").find("input #detail").val("");
		        $(this).parents(".layui-form").find("input #remark").val("");
		        $(this).prev().click();
		    });
